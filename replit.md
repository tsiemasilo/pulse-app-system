...
- **Agent Onboarding System (December 2025)**: Streamlined team leader onboarding workflow for new agents. Team leaders can submit onboarding requests with basic agent fields (firstName, lastName, email) and required organizational hierarchy (division, department, section). No username/password is required since agents don't log into the system. Requests are submitted for manager approval, and upon approval, agents are automatically created with auto-generated usernames (format: firstName_lastName) and assigned to the onboarding team leader's team. Features include: simplified form with cascading hierarchy selects (departments filter by division, sections filter by department), required division/department/section fields, manager approval/rejection workflow with rejection reasons, automatic username generation, automatic team assignment, comprehensive notifications to team leaders and managers, and request tracking.
