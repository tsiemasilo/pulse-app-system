...
export default function Reports({ user, teamMembers = [] }: ReportsProps) {
  ...
  const stats = {
    ...
    presentToday: new Set(attendanceData.filter(a => a.status === 'at work' || a.status === 'at work (remote)' || a.status === 'present').map(a => a.userId)).size,
    absentToday: new Set(attendanceData.filter(a => a.status !== 'at work' && a.status !== 'at work (remote)' && a.status !== 'present' && a.status !== 'late').map(a => a.userId)).size,
    lateToday: new Set(attendanceData.filter(a => a.status === 'late').map(a => a.userId)).size,
    attendanceRate: attendanceData.length > 0 
      ? ((new Set(attendanceData.filter(a => a.status === 'at work' || a.status === 'at work (remote)' || a.status === 'present').map(a => a.userId)).size / new Set(attendanceData.map(a => a.userId)).size) * 100).toFixed(1)
      : '0',
  };
  ...
  const teamPerformanceData = useMemo(() => {
    ...
    return filteredTeams.map((team) => {
      ...
      const presentCount = teamAttendance.filter(a => a.status === 'at work' || a.status === 'at work (remote)' || a.status === 'present').length;
  ...
  const attendanceTrendData = useMemo(() => {
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        present: dayAttendance.filter((a: any) => a.status === 'at work' || a.status === 'at work (remote)' || a.status === 'present').length,
        absent: dayAttendance.filter((a: any) => a.status !== 'at work' && a.status !== 'at work (remote)' && a.status !== 'present' && a.status !== 'late').length,
  ...
  const yesterdayData = useMemo(() => {
    ...
    return {
      present: yesterdayAttendance.filter((a: any) => a.status === 'at work' || a.status === 'at work (remote)' || a.status === 'present').length,
      rate: yesterdayAttendance.length > 0 
        ? ((yesterdayAttendance.filter((a: any) => a.status === 'at work' || a.status === 'at work (remote)' || a.status === 'present').length / yesterdayAttendance.length) * 100).toFixed(1)
        : '0'
    };
  }, [filteredHistoricalAttendance, selectedDate]);
