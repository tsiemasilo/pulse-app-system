...
export default function TeamLeaderView({ leaderId, isReadOnly, currentUser }: TeamLeaderViewProps) {
  ...
  const { data: leaderTeams = [] } = useQuery<Team[]>({
    queryKey: ["/api/teams/leader", leaderId],
    enabled: !!leaderId,
  });

  // Fetch team members for each team led by this leader
  ...
  // Fetch the team leader's information
  const { data: teamLeader = null } = useQuery<User | null>({
    queryKey: ["/api/users", leaderId],
    queryFn: async () => {
      if (!leaderId) return null;
      const response = await apiRequest("GET", `/api/users/${leaderId}`);
  ...
  // Fetch reporting manager information (for the team leader)
  const { data: reportingManager = null } = useQuery<User | null>({
    queryKey: ["/api/users", teamLeader?.reportsTo],
    queryFn: async () => {
      if (!teamLeader?.reportsTo) return null;
      const response = await apiRequest("GET", `/api/users/${teamLeader.reportsTo}`);
      return await response.json() as User;
    },
    enabled: !!teamLeader?.reportsTo,
  });
  ...
  const renderMainContent = () => {
    switch (activeTab) {
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
                <div>
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-2">
                    Welcome back, {teamLeader?.firstName || 'Team Leader'}
                  </h2>
      ...
      case 'reports':
        return <Reports user={teamLeader || currentUser} teamMembers={teamMembers} />;
      case 'employees':
        return (
          <div className="space-y-6 animate-in fade-in-50 duration-500">
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {isReadOnly && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-amber-100 dark:bg-amber-900/30 border-b border-amber-300 dark:border-amber-700 px-4 py-2">
          <div className="flex items-center justify-center gap-2">
            <Eye className="h-4 w-4 text-amber-700 dark:text-amber-400" />
            <p className="text-sm font-medium text-amber-800 dark:text-amber-300">
              View-Only Mode - You are viewing {teamLeader?.firstName} {teamLeader?.lastName}'s Dashboard
            </p>
          </div>
        </div>
      )}
      ...
      {/* Sidebar - Desktop */}
      <div className={`hidden lg:flex w-64 bg-white dark:bg-gray-800 shadow-lg border-r border-gray-200 dark:border-gray-700 flex-col h-screen sticky ${isReadOnly ? 'top-[40px]' : 'top-0'}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Header */}
        <header className={`bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700 ${isReadOnly ? 'mt-[40px]' : ''}`}>
